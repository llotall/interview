...
## Почему именно стек был выбран как структура для хранения информации о выховах методов?
...
- Каждый поток получает свой собственный стек памяти при создании. Это помогает изолировать выполнение потоков и позволяет безопасно управлять локальными переменными и параметрами методов для каждого потока.
...
## Отличие асинхронности от параллельности?
...
План запроса SQL (SQL Query Plan)
План запроса SQL, также известный как план выполнения (execution plan), представляет собой детализированное описание шагов, которые СУБД (система управления базами данных) выполнит для выполнения запроса. Понимание плана запроса помогает оптимизировать запросы и улучшить производительность базы данных.

## Зачем нужен план запроса?

- Оптимизация производительности: Анализируя план запроса, можно понять, как база данных обрабатывает запрос и какие индексы используются, что позволяет оптимизировать запросы 
и улучшить их производительность.
- Идентификация узких мест: План запроса помогает выявить потенциальные узкие места, такие как сканирование таблиц (table scans), которые могут замедлять выполнение запросов.
- Понимание поведения СУБД: План запроса дает представление о том, как СУБД выбирает и выполняет различные операции (например, соединение таблиц, фильтрация, сортировка), что 
помогает лучше понимать поведение базы данных.

```sql
EXPLAIN SELECT * FROM Employees WHERE Department = 'Sales';
EXPLAIN ANALYZE SELECT * FROM Employees WHERE Department = 'Sales';

-- ## Анализ статистики таблиц и индексов
-- Анализ статистики таблиц и индексов
-- Убедитесь, что статистика таблиц и индексов актуальна. Если данные изменились, PostgreSQL может принимать неверные решения об использовании индексов.
VACUUM ANALYZE table_name;

-- ## Проверить фрагментацию таблиц и индексов
-- Фрагментация может замедлить выполнение запросов. Используйте VACUUM для очистки таблиц и REINDEX для восстановления индексов.
...
Уровни изоляции транзакций определяют, как обрабатываются операции в транзакциях относительно других транзакций. Они устанавливают баланс между целостностью данных и производительностью. В зависимости от уровня изоляции, транзакции могут видеть изменения, сделанные другими транзакциями, по-разному. Рассмотрим основные уровни изоляции транзакций и их предназначение:
...
**Cпецифично для PostgreSQL**

- **GiST (Generalized Search Tree)**: Подходят для индексации геометрических данных и других сложных типов данных. Полезны для поиска ближайших соседей и диапазонных запросов на данных с пространственными компонентами.

- **SP-GiST (Space-Partitioned Generalized Search Tree)**: Полезны для специфических типов данных, таких как k-ближайшие соседи, маршрутизация и другие данные с пространственной структурой.
...
- **GIN (Generalized Inverted Index)**: Используются для индексации массивов, JSONB, tsvector и других типов данных, где необходима возможность быстрого поиска по множеству значений. Эффективны для полнотекстового поиска.

- **BRIN (Block Range INdexes)**: Подходят для очень больших таблиц, где данные имеют физическую кластеризацию. Используют меньше места и быстрее строятся, чем другие типы индексов, но менее эффективны для произвольных запросов.
